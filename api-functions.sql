...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        INSERT INTO user_word_progress (
            ...
            p_user_uuid, p_word_id, 'new', 0, 2.5, 1, NOW()
    ...
    ELSE
        -- Successful response
        IF v_current_progress.repetition_count = 0 THEN
            v_new_interval := 1;
        ELSIF v_current_progress.repetition_count = 1 THEN
            v_new_interval := 6;
        ELSE
        ...
        v_new_status := CASE 
            WHEN v_current_progress.learning_status = 'new' THEN 'learning'
            WHEN v_current_progress.repetition_count >= 2 AND v_quality >= 4 THEN 'learned'
            WHEN v_current_progress.repetition_count >= 5 AND v_quality = 5 THEN 'mastered'
    ...
    SET 
        ...
        repetition_count = CASE WHEN v_quality >= 3 THEN repetition_count + 1 ELSE repetition_count END,
    ...
    SELECT json_build_object(
        ...
        'next_review_date', NOW() + (v_new_interval || ' days')::INTERVAL,
        'interval_days', v_new_interval,
        'repetition_count', CASE WHEN v_quality >= 3 THEN v_current_progress.repetition_count + 1 ELSE v_current_progress.repetition_count END
    ) INTO v_result;
